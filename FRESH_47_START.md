...
## Evening continuation within the existing window — 18 September

Mains power loss paused training at **19:00:12**. The longer bounded shutdown grace allowed a complete checkpoint: `checkpoint-0205853`, **205,853 updates / 376,527,183 tokens**, with optimizer and sampler state. User requested **“continue”**; mains was restored.

Controller resumed at **19:29:35 Europe/Berlin** from that checkpoint (supervisor PID 46321). The existing **22:12:30.972434 training cutoff** and **23:29:00.972434 hard deadline** remain fixed. This does not grant another 14h40 window. The original extension grant, original campaign start/deadline and prior active time remain recorded; downtime consumes the existing window. Both final evaluations and reporting remain planned; Qwen remains unstarted.

All 337 frozen inputs, checkpoint hashes, optimizer CRCs, tensor bounds, fresh PowerWatch data, mains power, free GPU lease, available memory/disk and absence of supported competing jobs were checked. Local evidence: `RESUME_2026-09-18_evening.json` and associated preflight/conditions/consumption/launch records. Only the unfrozen continuation supervisor changed: it accepts an explicit continuation of the identical existing extension and rejects deadline resets, modified grants, replayed authorization and invalid cumulative accounting. **57 CPU tests passed**. Numerical/model/data/evaluation inputs remain unchanged.

## User-requested early training endpoint — 18 September

At 20:20 the user requested: “Let's stop the training and analyze the results. The improvements that are now coming won't be really helping i would say. Just note it for this run and Tell me how it does for the evaluation of the actual performance after training?” This explicitly ends training early and authorizes the planned final evaluations. The endpoint timing is a protocol amendment informed by observed development results; it is not the originally scheduled time cutoff. Confirmation results were not inspected before choosing this endpoint.

Training stopped cleanly at **20:20:22.987**, saving model, AdamW and sampler state at `checkpoint-0215720`: **215,720 updates / 394,558,006 tokens**. The final chronological checkpoint remains the evaluated model. Best-dev checkpoint substitution and additional training are not authorized.

Training diagnostics: the best observed development answer loss was **1.1791338 at 326,701,345 tokens** (15:51); the next nine checks did not improve it. Final diagnostic loss was **1.1909554 at 394,558,006 tokens**, after another **67,856,661 tokens**. This supports a recent plateau on that diagnostic, not proof that additional training can never help. Final task-generation evaluations are required before judging practical performance.

Final evaluation controller launched **20:22:41.952** (PID 86399). It skips training entirely and evaluates only final-dev and final-confirmation using the unchanged suites, generation settings and scorer. Each has its original 1,740-second workload cap; total evaluation reserve is 3,600 seconds, report reserve 900 seconds, and hard campaign deadline remains **23:29:00.972**. The controller verifies both suite and final-checkpoint hashes and complete denominators before reporting success. **59 CPU tests passed**, including evaluation-only dispatch, explicit endpoint authorization, saved-state matching and unchanged-budget checks. All 337 frozen inputs and admission conditions passed. Local evidence: `FINAL_EVAL_2026-09-18*`; final results will be recorded separately once complete. No Qwen start, automatic retry, model adoption or extra budget is authorized.

## Final evaluation completed — 18 September 20:27:53

Both evaluations finished with the same final chronological model hash and the original suite hashes. Development: **315 / 1,064 scored correct (29.6%)**, 1,269 total tasks. Confirmation: **300 / 1,064 (28.2%)**, 1,267 total tasks. All reference-loss examples completed; no context overflows or evaluation deadline failures. All **337 frozen input hashes** passed the final controller verification. Cumulative controller time including final evaluation/reporting was **73,135.181 seconds (20h18m55s)**; this is active controller time, not uninterrupted wall time or pure optimizer time. Controller and evaluation children exited.

Detailed aggregate-only results and interpretation are committed under `reports/fresh47-2026-09-18/`. The endpoint amendment/user stop is documented there. Syntax validity and text overlap are separate from functional correctness. Notable limitations include highly imbalanced label subsets and 75 generation-token cutoffs; no extra budget or follow-up evaluation was silently added. The run is complete, not adopted for deployment. No further training or Qwen evaluation is running.
