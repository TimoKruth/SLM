...
The prepared default is a randomly initialized 27.29M-parameter decoder with a new 16,384-token tokenizer trained only on the training partition. The proposed future campaign has a six-hour total cap, including checks, training, evaluation and reporting. It uses no old checkpoint or optimizer state.
...
Data, weights, generated responses and system process logs remain local and are excluded from Git. The new data version is `data/v7-benchmarks47-fresh-2026-09-16/`; the prepared campaign is `runs/fresh47-27m-2026-09-16/`.
