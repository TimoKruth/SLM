"""Bounded evaluation-only controller; children use the project's monitored launcher."""
import argparse
...
def main():
    ...
    with (run/'controller.lock').open('a') as lock:
        ...
        assert plan['mode'] == 'evaluation_only' and len(plan['jobs']) == 9
        ...
        state = dict(status='preflight', phase='verification', pid=os.getpid(), started=now(),
                     deadline=plan['deadline'], completed_evaluations=0, planned_evaluations=9,
        ...
        try:
            ...
            with GPULease() as lease:
                ...
                for job in plan['jobs']:
                    if cancelled():
                        raise RuntimeError('Requested stop or total deadline')
                    ...
                    command = [sys.executable, str(ROOT/'run_slm.py'), '--run', job['shadow'], '--module', 'slm.broad_eval',
                               '--monitoring', 'light', '--suite', str(run/'diagnostic-suite.json'), '--output', str(target),
                    ...
                    while child.poll() is None:
                        if cancelled() or time.monotonic()-started >= 630:
                            stop_child(child)
                    ...
                    assert protocol['suite_sha256'] == plan['inputs'][str(run/'diagnostic-suite.json')]
                    ...
                    child = None
                verify(plan['inputs'])
                emit(status='completed', phase='finished', finished=now(), frozen_inputs_unchanged=True)
