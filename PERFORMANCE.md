...
Aktuell bestehen **38 Performance- und Queue-Tests**: 35 unter `tests/performance/` (14 Runtime-, Vergleichs- und Schutztests, drei Projektstarter-Tests, zwei CPU-Trainingsäquivalenzfälle für `light` und `detail` und 16 zusätzliche Prozesssperren-/Queue-Fehlerfälle) sowie drei zusätzliche Queue-Schutztests unter `experiments/performance/test_queue.py`. Die Sitzungsnamen im Vergleichsbeispiel oben durch die tatsächlich erzeugten Ordnernamen ersetzen.
