...
def render(data):
    lines = ['# Performance-Messung', '',f"Status: {data['status']}; Modus: {data['mode']}; Prozesszeit: {data['elapsed_seconds']:.3f} s.",
             ...
             '', '| Phase | Aufrufe | Inklusiv s | Eigenzeit s | Mittel ms | p95 ca. ms |', '| --- | ---: | ---: | ---: | ---: | ---: |']
    for name,row in sorted(data['phases'].items(),key=lambda pair:pair[1]['self_seconds'],reverse=True):
        lines.append(f"| {name} | {row['count']} | {row['inclusive_seconds']:.6f} | {row['self_seconds']:.6f} | {row['mean_seconds']*1000:.3f} | {row['p95_seconds_approx']*1000:.3f} |")
